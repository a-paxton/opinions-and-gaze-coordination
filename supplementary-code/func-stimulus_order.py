def stimulus_order(xml_file):
    ...
    # clean them all up
    stimulus_order = [stimulus for stimulus in stimulus_order 
                      if len(re.findall('.rtf',stimulus))>0]
    stimulus_order = [stimulus for stimulus in stimulus_order 
                      if len(re.findall('instructions',stimulus))==0]
    stimulus_order = [stimulus for stimulus in stimulus_order 
                      if len(re.findall('thank-you',stimulus))==0]
